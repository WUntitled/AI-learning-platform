...
class LLMService:
    ...
    def _simulate_chat(self, messages: list, context: dict = None) -> str:
        ...
        # ===== 上下文感知：检查上一条assistant消息是否为追问引导 =====
        if len(messages) >= 2:
            prev = messages[-2]
            prev_content = prev.get("content", "")
            prev_role = prev.get("role", "")
            if prev_role == "assistant" and ("想深入了解" in prev_content or "如果您需要" in prev_content or "想了解哪" in prev_content or "想详细了解哪" in prev_content):
                return self._handle_follow_up(last_msg, messages, context)
        ...
        # ===== 默认回复 — 自然引导，不硬塞"如果您需要" =====
        return f"这是一个很好的问题！关于「{last_msg[:30]}」，我们可以从以下几个方面来探讨：\n\n首先，在电商业务分析中，理解业务背景是分析的基础。建议先明确这个问题的核心目标和影响因素。\n\n其次，可以结合数据分析方法和AI工具来辅助分析，比如使用对比分析、归因分析等方法。\n\n最后，将理论应用到实际业务中，通过不断实践来提升分析能力。\n\n您对哪个方面更感兴趣？我可以针对性地为您做详细讲解。"

    def _handle_follow_up(self, last_msg: str, messages: list, context: dict = None) -> str:
        """处理用户的追问回复 — 根据上次引导给出针对性深入讲解"""
        msg_lower = last_msg.lower()

        # 用户指定了要深入了解的类型
        if "GMV" in last_msg or "流量" in last_msg or "转化" in last_msg or "客单价" in last_msg:
            return "好的，我来深入讲解**GMV拆解分析**的方法：\n\n**1. 流量分析（访客数）**\n- 免费流量：搜索、首页、活动等渠道流量占比\n- 付费流量：直通车、钻展、联盟等的ROI\n- 流量质量：停留时长、跳出率、浏览深度\n\n**2. 转化分析（转化率）**\n- 商品详情页转化率\n- 加购率 → 下单率 → 支付率 三步漏斗\n- 各渠道的转化差异\n\n**3. 价格分析（客单价）**\n- 关联销售：搭配推荐提升客单价\n- 满减策略：满XX减XX促进凑单\n- 客单区间分布：主要成交价格带\n\n**4. 售后影响（退款率）**\n- 退款原因归类分析\n- 退款对最终GMV的影响\n\n需要我针对某个维度展开详细讲解吗？比如如何进行流量渠道的贡献度分析？"

        if "Prompt" in last_msg or "提示词" in last_msg:
            return "好的！我们深入讲解**Prompt撰写技巧**：\n\n**▎Prompt设计的核心框架：**\n\n**1. 角色设定（Role）**\n让AI扮演特定角色，例如：\n> 你是一名有5年经验的电商数据分析师\n\n**2. 任务描述（Task）**\n清晰说明要做什么：\n> 分析本月GMV下降的原因，从流量、转化、客单价三个维度进行\n\n**3. 上下文（Context）**\n提供充分的背景信息：\n> 本月比上月GMV下降15%，但广告费增加了20%\n\n**4. 输出格式（Format）**\n指定回答结构：\n> 用表格呈现各维度数据，最后给出分析结论\n\n**▎进阶技巧：**\n- **思维链（Chain-of-Thought）**：让AI分步思考\n- **Few-shot示例**：给AI一个参考示例\n- **迭代优化**：根据AI输出逐步完善Prompt\n\n您可以试着写一个Prompt，我帮您评估和优化！"

        if "对比分析" in last_msg or "归因" in last_msg or "漏斗" in last_msg or "分群" in last_msg:
            return "好的，我来详细讲解**数据分析方法**：\n\n**1. 对比分析**\n- 同比（Year-over-Year）：与去年同期比，消除季节性\n- 环比（Month-over-Month）：与上月比，反映短期趋势\n- 目标对比：与实际目标比，评估完成进度\n\n**2. 归因分析**\n- 多维度拆解：从流量渠道、商品品类、用户分层等维度\n- 贡献度计算：各因素对整体变化的贡献占比\n- 常用模型：加法归因、乘法归因\n\n**3. 漏斗分析**\n- 用户行为路径：曝光→点击→浏览→加购→下单→支付\n- 各环节转化率：找出流失最严重的环节\n- 优化策略：针对薄弱环节制定改进方案\n\n**4. 分群分析**\n- 按用户价值分层：RFM模型\n- 按行为特征分群：新客、活跃、沉睡、流失\n- 分层运营策略：不同群体不同策略\n\n您对哪种方法最感兴趣？我可以展开讲解具体操作步骤。"

        # 处理数字/排名类追问
        if any(kw in last_msg for kw in ["第一", "第二", "第三", "首先", "其次", "1", "2", "3"]):
            return "好的，我们逐一深入探讨！您提到的这个方面非常关键。\n\n在电商业务分析中，重要的是建立**数据驱动的思维方式**：\n1. **定义问题** — 明确要分析的核心业务问题\n2. **收集数据** — 获取相关的业务数据\n3. **分析诊断** — 运用合适的分析方法\n4. **制定方案** — 基于分析结果制定行动方案\n5. **跟踪反馈** — 持续跟踪效果并优化\n\n这五个步骤形成了一个完整的数据分析闭环。您想从哪个步骤开始深入了解？"

        # 没有匹配到具体方向时，回到之前的回答并进一步引导
        # 从最近的消息中提取主题
        recent_topics = []
        for m in reversed(messages):
            if m.get("role") == "user" and m.get("content"):
                ...
                if len(recent_topics) >= 2:
                    break
        topic_hint = recent_topics[-1][:20] if recent_topics else "这个方向"
        return f"明白了，关于「{topic_hint}」，我建议您可以尝试以下方式进一步学习：\n\n1. 在**AI学习助手**中继续提问，我会持续为您解答\n2. 前往**AI陪练助手**进行实战演练，巩固所学知识\n3. 通过**AI考试助手**测试自己的掌握程度\n\n您是想继续深入了解，还是想换个话题？"
